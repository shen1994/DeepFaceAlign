...
def NormRmse(N_LANDMARK, GroudTruth, Prediction):
...
def DAN(MeanShapeNumpy, IMGSIZE = 112, N_LANDMARK = 68):

    MeanShape = tf.constant(MeanShapeNumpy, dtype=tf.float32)
    InputImage = tf.placeholder(tf.float32,[None, IMGSIZE, IMGSIZE, 1], name="align_input")
    GroundTruth = tf.placeholder(tf.float32,[None, N_LANDMARK * 2])
    S1_Keepout = tf.placeholder(tf.float32, name="align_keepout1")
    S2_Keepout = tf.placeholder(tf.float32, name="align_keepout2")
    S1_isTrain = tf.placeholder(tf.bool, name="align_stage1")
    S2_isTrain = tf.placeholder(tf.bool, name="align_stage2")
    ...
    with tf.variable_scope('Stage1'):
        ...
        S1_DropOut = tf.layers.dropout(S1_Pool4_Flat, S1_Keepout, training=S1_isTrain)
        ...
        S1_Cost = tf.reduce_mean(NormRmse(N_LANDMARK, GroundTruth, S1_Ret))
    ...
    Ret_dict['S1_Keepout'] = S1_Keepout
    ...
    with tf.variable_scope('Stage2'):
        ...
        S2_DropOut = tf.layers.dropout(S2_Pool4_Flat, S2_Keepout, training=S2_isTrain)
        ...
        S2_Cost = tf.reduce_mean(NormRmse(N_LANDMARK, GroundTruth, S2_Ret))
    ...
    Ret_dict['S2_Keepout'] = S2_Keepout
